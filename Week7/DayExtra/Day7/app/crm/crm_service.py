...
class CRMService:
    ...
    def __init__(self):
        self.conn = sqlite3.connect(
            config.SQL_DATABASE_PATH, check_same_thread=False
        )
    ...
    def get_client_by_phone(self, phone: str) -> dict | None:
        ...
        # Normalized digit fallback (e.g. +923001234567 vs 03001234567)
        digits = "".join(c for c in phone if c.isdigit())
        if digits:
            short_digits = digits[-10:] if len(digits) >= 10 else digits
            all_clients = self._query("SELECT * FROM crm_clients ORDER BY created_at DESC")
            for client in all_clients:
                c_digits = "".join(c for c in client.get("phone", "") if c.isdigit())
                if c_digits and (c_digits.endswith(short_digits) or short_digits.endswith(c_digits[-10:] if len(c_digits) >= 10 else c_digits)):
                    return client

        return None

    def get_client_by_name(self, client_name: str) -> dict | None:
        rows = self._query(
            "SELECT * FROM crm_clients WHERE LOWER(client_name) = LOWER(?) ORDER BY created_at DESC LIMIT 1",
            (client_name,),
        )
        if rows:
            return rows[0]
