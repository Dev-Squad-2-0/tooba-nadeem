...
def intent_detection_node(state: GraphState) -> dict:
    ...
    if appt_intent == "cancel":
        ...
        appointment_intent.has_pending_details(session_id)
        and conv.last_intent in ("booking", "reschedule", "cancellation")
    ):
        # NEW: no appointment keyword matched THIS message, but the
        # session is already mid-collection (e.g. the buyer is just
        # answering "what's your name?" with a name -- no reason for
        # that answer to contain the word "book"). Stay on the same
        # appointment intent instead of falling through to
        # property_search, which would silently exit the guarded flow
        # and let the LLM improvise business-action language with no
        # deterministic_response_override protecting it.
...
def _is_valid_date(val: str | None) -> bool:
    """True only if val is a non-empty YYYY-MM-DD ISO string."""
...
def _is_valid_time(val: str | None) -> bool:
    """True only if val is a non-empty HH:MM 24-hour string."""
    if not val or not isinstance(val, str):
        return False
    import re
    return bool(re.fullmatch(r"\d{1,2}:\d{2}", val.strip()))
...
def check_missing_fields_node(state: GraphState) -> dict:
    ...
    # Validate date/time format — if the LLM returned a raw Urdu string like
    # "12 اگست" instead of "2026-08-12" (YYYY-MM-DD), treat it as missing so
    # the graph asks the buyer again rather than passing a malformed value to
    # Google Calendar which would throw and crash the entire turn.
    if "date" in details and not _is_valid_date(details.get("date")):
        _log_graph(
            f"check_missing_fields: date '{details.get('date')}' is not YYYY-MM-DD — treating as missing"
        )
        details.pop("date", None)
    if "time" in details and not _is_valid_time(details.get("time")):
        _log_graph(
            f"check_missing_fields: time '{details.get('time')}' is not HH:MM — treating as missing"
        )
...
def cancellation_node(state: GraphState) -> dict:
    ...
    if not details.get("client_name"):
        note = "Buyer wants to CANCEL their appointment but we don't have their name yet. Ask for it."
        return {"appointment_status_note": note}

    result = biz_tools.cancel_appointment_tool(
        phone=details.get("phone", "Unknown"),
        client_name=details["client_name"],
        notes=details.get("notes")
    )
