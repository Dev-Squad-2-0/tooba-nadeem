...
@router.post("/chat/completions/chat/completions")
def chat_completions(raw_request: Request, request: ChatCompletionRequest):
    ...
    # Extract caller-ID phone number, if Vapi supplied one. For a
    # phone-originated call, Vapi passes the caller's number (E.164
    # format, taken from the SIP "From" header) as call.customer.number
    # on every request -- see https://docs.vapi.ai/assistants/dynamic-variables.
    # Not present for text-only/testing calls through this same endpoint;
    # appointment_intent.py falls back to asking for the phone number by
    # voice in that case.
    caller_phone = None
    if request.call and isinstance(request.call, dict):
        customer = request.call.get("customer")
    ...
    if len(user_msgs) <= 1:
        ...
        # Clear any stale caller-ID phone before re-seeding below, in
        # case this session_id were ever reused across calls.
        appointment_intent.clear_caller_phone(session_id)

    # Safe to call every turn (idempotent) -- keeps the caller's phone
    # on file even after clear_pending() wipes it post-booking, e.g. a
    # buyer who books then immediately asks to reschedule in the same call.
    if caller_phone:
        appointment_intent.set_caller_phone(session_id, caller_phone)
