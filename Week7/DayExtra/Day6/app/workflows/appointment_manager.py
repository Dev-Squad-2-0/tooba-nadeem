...
def reschedule_appointment(
    ...
    if not client:
        logger.warning("[RESCHEDULE] FAILED: no client found for phone=%s", phone)
        return {
            "success": False, "appointment": None,
            "error": (
    ...
    if agent_email:
        try:
            ...
            logger.info("[RESCHEDULE] Notification email SUCCESS")
        except EmailServiceError as exc:
            logger.warning("[RESCHEDULE] Notification email FAILED (event still updated): %s", exc)
