...
# NEW — OpenRouter-only: server-side fallback if the free slug 404s from
# shared rate-limit exhaustion. Paid, but cheap ($0.10/$0.32 per M tokens),
# and it's a non-reasoning model like the primary — so no leakage risk.
# See client.py, where this is passed as extra_body={"models": [...]}.
